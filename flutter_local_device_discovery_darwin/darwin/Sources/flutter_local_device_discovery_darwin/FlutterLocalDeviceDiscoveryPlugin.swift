...
class DiscoverySession {
  ...
  func start() {
    // First, browse the meta-service "_services._dns-sd._udp" to discover
    // ALL service types available on the network. This finds devices that
    // advertise services we didn't explicitly request.
    let parameters = NWParameters()
    parameters.includePeerToPeer = true
    let metaBrowser = NWBrowser(
      for: .bonjour(type: "_services._dns-sd._udp", domain: nil),
      using: parameters
    )

    metaBrowser.stateUpdateHandler = { [weak self] state in
    ...
    metaBrowser.browseResultsChangedHandler = { [weak self] results, changes in
      self?.handleMetaBrowseResultsChanged(results: results, changes: changes)
    }

    metaBrowser.start(queue: .global(qos: .userInitiated))
    browsers.append(metaBrowser)

    // Also browse the explicitly requested service types directly.
    for serviceType in serviceTypes {
